...
const getStation = async()=>{
    ...
    const response = await fetch("/FireFighting/stations/" + station_id);
...
//get Engineer's name
 const getEngineer = async ()=>{
    ...
    const response = await fetch("/FireFighting/getEngineer/" + area_id + "/" + shift_id);
    if (response.status !== 200) {
        throw new Error('can not fetch the data');
    }
    const data = await response.json();
...
let eng_id = engineerSelect.value;
    const response = await fetch("/FireFighting/getEngineersEmail/" + eng_id);
    if (response.status !== 200) {
        throw new Error('can not fetch the data');
    }
    const data = await response.json();
...
const getEngineersShift = async ()=>{
    ...
    const response = await fetch("/FireFighting/getEngineersOnShift/"+ area_id+'/'+ shift_id);
    if (response.status !== 200) {
        throw new Error('can not fetch the data');
    }
    const data = await response.json();

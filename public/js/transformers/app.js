...
switch(check){
    case 'TroubleShooting':
        ...
        MechAlarm_select_option3.value = 'Remove Fan';
        ...
        case 'Inspection':
          ...
          MechAlarmSelect.add(MechAlarm_select_option3)
          //option 4
          MechAlarm_select_option4.text = 'Check MCB(Main - LT-Cooling Panel MCB)';
          MechAlarm_select_option4.value = 'Check MCB(Main - LT-Cooling Panel MCB)';
          MechAlarmSelect.add(MechAlarm_select_option4)
            break;
...
switch(check){
    case 'Emergency':
        ...
        case 'Inspection':
            //option 1
            chemistryAlarm_select_option.text = 'Oil Sampling for Laboratory Routine Program';
            chemistryAlarm_select_option.value = 'Oil Sampling for Laboratory Routine Program';
            //option 2
            ...
            //option 3
            chemistryAlarm_select_option3.text = 'Oil sampling for newly arrived oil drums';
            chemistryAlarm_select_option3.value = 'Oil sampling for newly arrived oil drums';
            //option 4
            chemistryAlarm_select_option4.text = 'Testing and Suppling of Oil Drumbs Prior to Distribution';
            chemistryAlarm_select_option4.value = 'Testing and Suppling of Oil Drumbs Prior to Distribution';
...
//get Engineer's name
 const getEngineer = async ()=>{
    ...
    const response = await fetch("Transformers/getEngineer/" + area_id + "/" + shift_id);
    if (response.status !== 200) {
        throw new Error('can not fetch the data');
    }
    const data = await response.json();
...
let eng_id = engineerSelect.value;
    const response = await fetch("Transformers/getEngineersEmail/" + eng_id);
    if (response.status !== 200) {
        throw new Error('can not fetch the data');
    }
    const data = await response.json();
...
const getEngineersShift = async ()=>{
    ...
    const response = await fetch("/getEngineersOnShift/"+ area_id+'/'+ shift_id);
    if (response.status !== 200) {
        throw new Error('can not fetch the data');
    }
    const data = await response.json();

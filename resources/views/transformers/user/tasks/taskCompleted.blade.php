                    ...
                    <table id="example1" class="table key-buttons text-md-nowrap" data-page-length='50'>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$i}}</td>
                                <td>
                                   <a href="{{route('Transformers.user.veiwReport',['id'=>$task->task_id])}}">{{$task->tasks->refNum}}</a>
                                ...
                                <td><a href="{{route('Transformers.user.taskDetails',['id'=>$task->tasks->id])}}"
                                        class=" btn btn-info">Details</a></td>
                                <td>
                                    @if($task->status == 'pending')
                                    <a href="{{route('Transformers.engineerReportForm',['id'=>$task->tasks->id])}}"
                                        class="btn btn-danger">Action
                                        Take</a>
                                </td>
                                @endif
                                @if($task->status == 'completed')
                                <a href="{{route('Transformers.user.veiwReport',['id'=>$task->task_id])}}"
                                    class="btn btn-success">Report</a>
                                </td>
                                @switch($task->report_status)
                                @case(1)
                                <td> <a href="{{route('Transformers.requestEditReport',['id'=>$task->task_id])}}"
                                ...
                                @break
                                @case(0)
                                <td> <a href="{{route('Transformers.editReport',['id'=>$task->task_id])}}"
                                        class="btn btn-info"> تعديل</a>
                                </td>
                                @break
                                @default
                                <td> <a href="{{route('Transformers.requestEditReport',['id'=>$task->id])}}"

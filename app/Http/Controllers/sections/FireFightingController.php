...
class FireFightingController extends Controller
{
      ...
      public function index(){
        ...
        return view('firefighting.admin.dashboard',compact('tasks','task_details','date','monthName'));

    }
     //// start front END functions
     public function add_task(){
        $stations = Station::all();
        return view ('firefighting.admin.tasks.add_task',compact('stations'));
    }
    ...
    public function getEngineersEmail($eng_id){
        return (string) Engineer::where("section_id",4)
        ->where('id',$eng_id)
        ->first();
    }
      ...
      public function store(Request $request){ 
        ...
        if ($request->hasfile('pic')) {
            ...
            foreach ($request->file('pic') as $file) {
                ...
                $file->move(public_path('Attachments/firefighting/' . $task_id), $name);
                $data[] = $name;
                $refNum = $request->refNum;
    ...
    public function showAllTasks(){
        $tasks = Task::where('fromSection',4)->orderBy('id', 'desc')
        ->get();
        return view('firefighting.admin.tasks.showTasks',compact('tasks'));
    }

    public function showPendingTasks(){
        $tasks = Task::where('fromSection',4)
        ->where('status','pending')
        ->orderBy('id', 'desc')
        ->get();
        return view('firefighting.admin.tasks.showTasks',compact('tasks'));  
    ...
    public function userArchive(){
        ...
        ->orderBy('id', 'desc')
        ->get();
        return view('firefighting.user.tasks.showTasks',compact('tasks'));
    }
    ...
    public function taskDetails($id){
        ...
        return view('firefighting.admin.tasks.taskDetails',compact('tasks','task_details','task_attachment'));
    }

    public function showEngineers(){
        $engineers = Engineer::where('section_id',4)->get();
        return view ('firefighting.admin.engineers.engineersList',compact('engineers'));
    }
    ...
    public function updateTask($id){
        ...
        return view('firefighting.admin.tasks.updateTask',compact('tasks','stations','task_attachments'));
    }
...
//post
    ...
    public function open_file($id, $file_name){
        $files = Storage::disk('public_uploads')->getDriver()->getAdapter()->applyPathPrefix('firefighting/'.$id . '/' . $file_name);
        return response()->file($files);
    }
    public function get_file($id, $file_name)
    {
        $contents = Storage::disk('public_uploads')->getDriver()->getAdapter()->applyPathPrefix('firefighting/'.$id . '/' . $file_name);
    ...
    public function viewPrintReport($id){
        ...
        ->where('status','completed')
        ->first();
        return view('firefighting.admin.tasks.report',compact('task_details'));
    }
    ...
    public function userIndex() {
        ...
        ->paginate(4);
        $date = Carbon::now();
        return view('firefighting.user.dashboard',compact('tasks','task_details','date','engineers','stations'));
    }
    ...
    public function engineerPageTasks($id){
        ...
        $tasks = Task::where('eng_id',$engineer)
            ->orderBy('id', 'desc')
            ->get();
        return view('firefighting.user.mytasks', compact('tasks'));
    }
    public function usertaskDetails($id){
        $tasks = Task::where('id', $id)->first();
        $task_details = TaskDetails::where('task_id', $id)->get();
        $task_attachment = TaskAttachment::where('id_task', $id)->get();
        return view('firefighting.user.tasks.taskDetails',compact('tasks','task_details','task_attachment'));
    }
    ...
    public function engineerReportForm($id){
        $tasks = Task::where('id',$id)->first();
        return view('firefighting.user.EngineerReportForm',compact('tasks'));
    }
    ...
    public function SubmitEngineerReport(Request $request,$id){
        ...
        if ($request->hasfile('pic')) {
            foreach ($request->file('pic') as $file) {
                $name = $file->getClientOriginalName();
                $file->move(public_path('Attachments/firefighting/' . $id), $name);
                $data[] = $name;
                $refNum = $request->refNum;
                $attachments = new TaskAttachment();
                $attachments->file_name = $name;
